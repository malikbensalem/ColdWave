...
## Iteration 28 (2026-08-06) — Custom voice preview, full CRM lead editing, DNC removal
- **Custom voice usability**: `get_voice`/`generate_tts`/`_relay_tts`/`generate_voice_preview` already resolve the pasted `elevenlabs_custom_voice_id`. Added a **"Save & preview custom voice"** button in Settings → Integrations (saves then plays a sample via `/voices/preview` with `voice_id="custom"`), plus an amber note that **live Twilio calls require the ElevenLabs API key linked in the Twilio Console** (custom/cloned voices). Previews & test calls work with the key saved in-app. (User's ID `rfkTsdZrVWEVhDycUYn9` will work once ElevenLabs is enabled + the voice selected.)
- **Edit existing leads**: new **Edit** button in the CRM detail sheet → dialog with all fields → `PUT /contacts/{id}`. `ContactUpdate` extended with first/last name + all lead fields.
- **Full fields when adding**: `ContactCreate` extended (first/last name, pipeline status, lead_status/owner/owner_alias/source, traffic category, notes, consent); `create_contact` builds `name` from first+last when blank. New Lead + Edit share one `LeadFields` component. Verified via curl (create-all-fields, edit).
- **Take a lead off Do-Not-Call**: new `POST /contacts/{id}/allow-calling` clears DNC + resets `opted_out=False`, `status="new"`. CRM detail shows an **"Allow calling"** button (green) instead of "DNC" when the lead is opted-out/DNC. `remove_dnc` (Compliance page) now also re-enables any matching contact. Verified: DNC→allow-calling resets to callable.

## Iteration 29 (2026-08-06) — ElevenLabs Explore/library voice handling
- **Root cause (not a code bug)**: voice `rfkTsdZrVWEVhDycUYn9` is a **shared ElevenLabs Explore library voice**. ElevenLabs returns HTTP 402 `paid_plan_required` — free plans cannot use library voices via the API. The code already passes any voice ID dynamically (works on paid plans); the failure was the free-plan restriction surfaced as a raw error dump.
- **Fix**: `generate_tts` now parses ElevenLabs SDK `ApiError` (`_parse_elevenlabs_error`) and returns clear, actionable guidance (`_friendly_tts_error`) instead of a header dump — distinct messages for 402 (Explore/library → add to My Voices or upgrade), 401 (bad key), 404 (voice not in account). Preview/test-call surface this message. Returns `error_code`/`status` too.
- **UI guidance (Settings → ElevenLabs)**: added instructions — to use an Explore voice, "Add to My Voices" then paste the Voice ID from My Voices; to swap voices later, just paste a different Voice ID and Save. Amber note explains the free-plan library restriction + Twilio Console linkage for live calls.
- **Swapping voice IDs**: no code change ever needed — the Custom Voice ID field in Settings dynamically accepts any account voice ID.

## Iteration 30 (2026-08-06) — Call-agent quality overhaul + Gemini Flash-Lite fix
- **Gemini "Internal Server Error: AI Agent" FIXED**: `gemini-2.5-flash-lite` is NOT a valid model for the Emergent LLM key (litellm rejects it → 500). Removed it from `LLM_MODELS['gemini']`; default is now `gemini-2.5-flash` (valid, fast). `llm_config` already validates the saved model against the list and falls back to default, so any stale `flash-lite` selection self-heals; also migrated existing orgs in the DB. Verified end-to-end through `/api/calls/test/start` + `/turn` (Gemini reply returned, no error). Valid Gemini set: gemini-2.5-flash, gemini-3-flash-preview, gemini-3.5-flash, gemini-3.1-pro-preview, gemini-2.5-pro.
- **STT accuracy + contextual vocabulary**: expanded ConversationRelay `hints` with restaurant/Flipdish/hospitality/sales terms (Flipdish, online ordering, boosting orders, point of sale/POS, kiosk, commission, Deliveroo/Just Eat/Uber Eats, average order value, click and collect, etc.) + brand name — biases transcription toward the right words. Verified in TwiML.
- **Semantic correction + graceful uncertainty**: system prompt now tells the agent the input is live STT that may contain errors, to interpret by MEANING (e.g. "basting orders"→"boosting orders", "flip fish"→"Flipdish", "point of scale"→"point of sale"), to use the FULL conversation and not re-ask answered questions, and to ask ONE brief clarifying question when genuinely unsure instead of guessing.
- **Interruption / end-of-speech**: the WS handler now accumulates partial `prompt` chunks and only generates a reply on the FINAL transcript (`last=true`), so the AI no longer talks over the caller or answers a half-heard sentence; any new speech cancels the in-flight reply (barge-in).
- **Latency**: final-gating removes wasted cancelled LLM calls on partials; kept warm-up + token streaming + sentence chunking; fast default model.
- **Call termination hardened**: ends ONLY on agent goodbye, explicit opt-out, or a silence watchdog — after `relay_silence_seconds` (default 30s) of silence it nudges once ("Are you still there?"), then politely closes after another window. No unexpected mid-conversation hang-ups.
- ⚠️ Live-call behaviours (STT/latency/interruption/silence) can only be fully confirmed on a real deployed Twilio call; Gemini fix + hints + prompt logic verified in-preview.

## Iteration 31 (2026-08-06) — Who-ended-the-call, callback tagging, no-answer tagging
- **`ended_by` recorded** on every call at each termination point: `agent` (AI said goodbye), `prospect_optout` (opted out), `prospect_hangup` (WS closed without an intentional close), `silence_timeout` (silence watchdog), `voicemail` (AMD), and `busy`/`no_answer`/`failed`/`canceled` (Twilio status). Shown in the CRM call-history card ("Ended by: …") and the live CallMonitor. Verified via status callback (busy→ended_by=busy).
- **Callback tagging**: scheduling a callback (`PUT /contacts/{id}` with `callback_at`) now moves the lead to the `callback` stage AND tags their most recent answered call `is_callback=True, outcome="callback"` (keeping its rating). CRM shows a blue **Callback** badge. Verified (rating preserved + tagged).
- **No-answer/voicemail tagging (unchanged intent, expanded)**: voicemail (AMD), suspected voicemail (connected but prospect never spoke — greeting only), no-answer/busy/failed all set `status="no_answer"`, an appropriate `outcome`, and are **not rated**. New **Suspected voicemail** badge added alongside Voicemail/No-answer. Verified voicemail + busy paths.

## Iteration 32 (2026-08-27) — Telnyx (telephony) + Inworld (STT+TTS) provider option
- **New per-org providers, UI-configurable, no env/self-host**. Twilio/3CX/ElevenLabs remain unchanged fallbacks.
- **Model** (`models.py` IntegrationSettings): `telnyx_*` (api_key, public_key, connection_id, phone_number, enabled), `inworld_*` (api_key, tts_voice_id, tts_model default `inworld-tts-2-flash`, stt_enabled, enabled), `telephony_provider` now `3cx|twilio|telnyx`, new `tts_stt_provider` = `elevenlabs|browser|inworld`. Secrets added to `audit.SECRET_FIELDS`.
- **`telnyx_voice.py`** (new): outbound via Call Control `POST /v2/calls` (connection_id + from, AMD `detect`), Ed25519 webhook signature validation (pynacl), `/telephony/telnyx/webhook` handling call.initiated/answered/hangup/machine.detection.ended → **starts Media Streaming on answer, short-circuits AI + hangs up on voicemail** (`telnyx_is_machine`), `telnyx_validate` for key/connection test.
- **`telnyx_relay.py`** (new): our own bridge — Telnyx bidirectional Media WS (8k PCMU) ↔ Inworld Realtime STT (VAD-gated, mu-law→PCM16→16k via audioop) → existing `stream_agent_reply` LLM → Inworld Realtime TTS (MULAW 8k) streamed back, with barge-in cancel.
- **`integrations.py`**: `select_stt_provider`, extended `select_tts_provider` (inworld/browser/elevenlabs), `generate_tts_inworld` (WS, streaming), `inworld_stt_stream`, `inworld_list_voices` (dynamic + static fallback w/ reason), `inworld_ping`, Mongo-backed **TTS cache** (`tts_cache` collection, localhost-portable) with hash key `(org_id,voice,model,text)`.
- **`routes.py`/`telephony.py`**: `place_outbound_call` routes by `telephony_provider` (+Telnyx branch, validates required fields), auto-dial/dial validation includes Telnyx; new `POST /settings/integrations/{telnyx,inworld}/test` and `GET /inworld/voices`.
- **`server.py`**: mounts `build_telnyx_voice_router` + `build_telnyx_relay_router`.
- **Settings UI**: Telephony Provider switch (3CX|Twilio|Telnyx) + Voice AI Provider switch (ElevenLabs|Browser|Inworld) with live per-minute cost hints; full Telnyx section (+ copyable webhook URL) and Inworld section (dynamic voice picker, model select w/ "fastest & cheapest", STT/TTS toggles, Test).
- **Tests**: `tests/test_telnyx_inworld.py` — 21 passing (Ed25519 valid/invalid, provider resolution across combos, TTS cache hit/miss & key determinism, AMD short-circuit).
- **Verified live (preview)**: Telnyx `/test` (valid ✓ against real API), Inworld `/test` (valid ✓), `/inworld/voices` (100 dynamic voices), Inworld TTS synth + cache hit, provider selection, Settings UI renders. ⚠️ End-to-end live audio (media bridge, barge-in, real UK call to +447754775233) requires deployment — public webhook + WS reachability — so must be validated after deploy. Test creds were used only in the demo org and cleared afterward; nothing hardcoded.

## Iteration 33 (2026-08-27) — Voices provider-aware + Telnyx relay TTS branching + warm cache (bug fix)
- **`GET /api/voices` now provider-aware** (`routes.py list_voices`): when `tts_stt_provider == "inworld"` + key set, returns Inworld voices normalized to the ElevenLabs UI shape (`id/name/gender/accent/display_name/persona/speed/stability/style/dynamic`, `provider:"inworld"`); otherwise the ElevenLabs `VOICE_CATALOG` + custom voice with top-level `provider:"elevenlabs"`. Fixes Campaigns Voices tab / voice dropdowns not showing Inworld voices.
- **`telnyx_relay.py` TTS/STT now branch on org settings**: TTS via new `generate_tts_telephony(org,text,voice_id)` → Inworld (`generate_tts_inworld`, MULAW 8k) or ElevenLabs (`output_format=ulaw_8000`), both using the Mongo TTS cache. STT via `select_stt_provider` (Inworld only; degrades gracefully with a clear log when not selected/keyed).
